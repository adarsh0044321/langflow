            ...
            while GetMessageW(&mut msg, HWND(0), 0, 0).as_bool() {
                if msg.message == WM_HOTKEY {
                    ...
                    match hotkey_id {
                        ...
                        HOTKEY_TYPING_ID => {
                            // Toggle the real-time inline typing translation mode
                            let handle = app_handle.clone();
                            toggle_inline_typing_mode(handle);
                        }
                        _ => {}
                    }
                }
...
pub fn toggle_inline_typing_mode(app: AppHandle) {
    ...
    // Update the active IME state
    crate::core::ime::set_ime_enabled(config.inline_typing_enabled);

    // Emit event to notify frontend (e.g. to show status changes)
